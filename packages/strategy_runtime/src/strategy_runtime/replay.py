...
ZERO = Decimal("0")
ONE = Decimal("1")
DEFAULT_MANIFEST = TrendRider().manifest
...
class ReplayConfig:
    ...
    monthly_breakers_enabled: bool = True
...
class ReferenceReplayEngine:
    ...
    def __init__(
        ...
        *,
        ...
        if self.start_i < max(200, config.parameters.slow_period):
    ...
    def _open_long(self, price: Decimal, atr: Decimal, timestamp: pd.Timestamp) -> None:
        ...
        self.position = Position(
            ...
            tp1=round_price(
                price + Decimal(str(engine.TP1_R)) * stop_distance,
    ...
    def _process_intrabar(self, i: int, row: pd.Series[Any]) -> None:
        ...
        if not position.tp1_done and position.tp1 is not None and high >= position.tp1:
            ...
            tp_qty = _production_tp_qty(
                position.qty,
                Decimal(str(self.config.parameters.tp1_frac)),
                self.filters,
            )
            if tp_qty > 0:
    ...
    def _close_position(self, price: Decimal, timestamp: pd.Timestamp, reason: str) -> None:
        ...
        if position.side == "LONG":
            # Pullback memory accumulated while a long was open cannot arm the
            # next long. The production plugin begins looking again only after
            # this close, using last_long_closed_at_ms as its boundary.
            self.was_below = False
        self.position = None
    ...
    def _update_breakers(
        ...
        if not self.config.monthly_breakers_enabled:
            return
    ...
    def _update_signal_memory(self, row: pd.Series[Any], prev: pd.Series[Any]) -> None:
        if not bool(row["regime"]):
            # The plugin searches pullbacks only inside the current bull-regime
            # run; a regime break invalidates older below-EMA20 observations.
            self.was_below = False
        elif float(row["close"]) < float(row["ema20"]):
            self.was_below = True
...
class PluginReplayEngine(ReferenceReplayEngine):
    ...
    def __init__(
        ...
        *,
        ...
        self.strategy = TrendRider(config.parameters, config.interval)
        self.df["sleeve_realized_vol"] = self.df["close"].pct_change().fillna(0.0).ewm(
            span=config.parameters.sleeve_vol_span, adjust=False
        ).std() * np.sqrt(self.strategy.bars_per_year)
    ...
    def _process_open(self, i: int, row: pd.Series[Any], prev: pd.Series[Any]) -> None:
        ...
        intents = self.strategy.on_prepared_frame(
            self.df.iloc[:i],
            state,
        )
        ...
        for intent in intents:
            if isinstance(intent, ExitAll) and self.position is not None:
                ...
                self.position.stop = max(
                    self.position.stop,
                    round_price(Decimal(str(intent.price)), self.filters.tick_size),
                )
            elif (
    ...
    def _open_short_from_weight(
        ...
        equity = self._mark_equity(price)
        raw_qty = margin_capped_qty(
            equity * weight / price,
            price,
            equity,
            self.config.leverage_cap,
        )
        qty = clamp_qty(raw_qty, self.filters)
