...
test("REF-01 select pinned refined release, retain DEMO and stopped state, restore original", async ({ page }) => {
  await login(page);
  await page.goto("/settings");
  await expect(page.getByTestId("active-env")).toContainText("DEMO");
  const card = page.getByTestId(`strategy-${refined}`);
  await expect(card).toContainText("Refined live settings · 4h");
  await card.getByText("How this strategy works", { exact: true }).click();
  await expect(page.getByTestId(`parameters-${refined}`)).toContainText("trail atr: 4.5");
  ...
  await page.getByTestId(`select-${refined}`).click();
  await dialog.getByRole("button", { name: "Select release", exact: true }).click();
  try {
    ...
    const events = await page.request.get("/api/events?category=strategy", { headers });
    expect((await events.json()).items.some((event: { payload_json: { to?: string } }) =>
      event.payload_json?.to === refined)).toBeTruthy();
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
  } finally {
    await page.getByTestId(`select-${original}`).click();
    await page.getByRole("dialog").getByRole("button", { name: "Select release", exact: true }).click();
    await expect(page.getByTestId(`strategy-${original}`).getByText("Active", { exact: true })).toBeVisible();
  }
});
